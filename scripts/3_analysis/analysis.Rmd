...
```{r filtered support weekly data}
panel_community <- data %>%
  filter(
    city %in% c("Eugene", "Springfield"),
    timestamp >= as.POSIXct("2023-01-01"),
    nature %in% top_14_cahoots_natures
  ...
  mutate(
    ...
    treated   = as.integer(city == "Eugene"),
    post      = as.integer(timestamp >= date_cahoots_stop),
    community = as.integer(str_detect(
      str_to_lower(as.character(outcome)),
      "assist|advised|resolved|counseling|welfare check done|remained in community"
    ))
  ) %>%
  group_by(city, week, treated) %>%
  summarise(
    n_calls        = n(),
    community_rate = sum(community, na.rm = TRUE) / n(),
    post           = max(post),
    .groups = "drop"
  ) %>%
  mutate(week_num = as.numeric(difftime(week, min(week), units = "weeks")))
...
```{r lineplot community support rate}

vline_date <- as.Date("2025-04-07")

parallel_trends_community <- panel_community %>%
  group_by(city) %>%
  arrange(week) %>%
  mutate(rate_smooth = zoo::rollmean(community_rate, k = 4, 
                                     fill = NA, align = "right")) %>%
  ggplot(aes(x = week, y = rate_smooth, color = city)) +
  geom_line(linewidth = 0.9) +
  geom_vline(xintercept = as.POSIXct(vline_date),
             linetype = "dashed", color = "red", linewidth = 0.8) +
  annotate("text", x = as.POSIXct(vline_date),
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title    = "Community Support Rate by City",
    subtitle = "4-week rolling average for the 14 most frequent CAHOOTS call natures",
    x = NULL, y = "Community Support Rate", color = "City"
  ) +
  theme_minimal()

parallel_trends_community
```

### Linear trends

```{r linear trends community support}
linear_trends_community <- panel_community %>%
  ggplot(aes(x = week, y = community_rate, color = city)) +
  geom_line(alpha = 0.3, linewidth = 0.4) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 1) +
  geom_vline(xintercept = as.POSIXct("2025-04-07"),
             linetype = "dashed", color = "red") +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(
    title    = "Community Support Rate, Linear Trend by City",
    subtitle = "Free y-axis scale per city",
    x = NULL, y = "Community Support Rate", color = "City"
  ) +
  theme_minimal()

linear_trends_community
```
...
```{r parallel trends check support}
# Parallel trends check
pre_community <- panel_community %>% filter(post == 0)
trend_test_community <- lm(
  community_rate ~ week_num * treated,
  data    = pre_community,
  weights = n_calls
)
stargazer(trend_test_community,
          type = "text")
```
...
```{r}
did_community <- lm(
  community_rate ~ treated * post,
  data    = panel_community,
  weights = n_calls
)

stargazer(did_community,
          type  = "text",
          title = "DiD, Effect of CAHOOTS Discontinuation on Community Support Rate (Weekly)",
          covariate.labels = c("Eugene (Treated)",
                               "Post-Discontinuation",
                               "DiD Estimator (Treated × Post)"),
          dep.var.labels = "Community Support Rate",
          notes = "OLS weighted by weekly call volume.")
```
