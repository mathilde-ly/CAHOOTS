...
Did CAHOOTS' discontinuation induce and increase in arrests in Eugene?


First we harmonize the data by calculating the weekly rolling average arrest rate per week for Eugene and Springfield, using only CAHOOTS call types (agency is not necessarily CAHOOTS).
...
H0 : both groups are the same before treatment
H1 : trends are different

we don't want to reject null hypothesis so p-value should be over 0.5 (for 95% certainty)

Here p-value is 0.544 so the parallel trends passed, we can proceed with the DiD method.

```{r stat test parallel trends}

pre_cahoots <- panel_cahoots_scope %>% filter(post == 0)
