...
# Semaine de discontinuation pour le repère visuel
vline_date <- as.Date("2025-04-07")

parallel_trends_check <- panel %>%
  # Lisser par moyenne mobile 4 semaines pour la lisibilité
  group_by(city) %>%
  arrange(week) %>%
  ...
  labs(
    title    = "Arrest Rate by City, Parallel Trends Check",
    subtitle = "4-week rolling average for most frequent CAHOOTS call natures",
    x = NULL, y = "Arrest Rate", color = "City"
  ) +
  theme_minimal() 

ggsave("../../figures/analysis/parallel_trends_check.png", parallel_trends_check, width = 10, height = 5, dpi = 180)
...
panel %>%
  ...
  scale_color_manual(values = c("Eugene" = "#2ACCFE", "Springfield" = "#FEA4E6")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(title = "Raw arrest rate and linear trend by city",
...
pre_cahoots <- panel %>% filter(post == 0)
...
# --- Variable de période relative ---
treatment_week_num <- panel %>%
  filter(week >= as.Date("2025-04-07")) %>%
  pull(week_num) %>% min()

panel <- panel %>%
  mutate(rel_week = week_num - treatment_week_num)

# --- TWFE baseline ---
twfe <- feols(
  arrest_rate ~ i(post, treated, ref = 0) | city + week_num,
  data    = panel,
  weights = ~n_calls,
  cluster = ~city
)
...
# --- Event study ---
event_study <- feols(
  arrest_rate ~ i(rel_week, treated, ref = -1) | city + week_num,
  data    = panel,
  weights = ~n_calls,
  cluster = ~city
)
...
iplot(event_study,
      xlab  = "Weeks relative to CAHOOTS discontinuation",
      ylab  = "Estimated effect on arrest rate",
      main  = "Event Study — Effect of CAHOOTS Discontinuation on Arrest Rate",
      ref.line = 1)
...
panel_monthly <- data %>%
  ...
  mutate(
    ...
    treated = as.integer(city == "Eugene"),
    post    = as.integer(timestamp >= date_cahoots_stop),
    arrest  = as.integer(str_detect(str_to_lower(as.character(outcome)),
                "arrest|juvenile taken into custody"))
  ) %>%
  group_by(city, month, treated) %>%
  summarise(
    n_calls     = n(),
    n_arrests   = sum(arrest, na.rm = TRUE),
    arrest_rate = n_arrests / n_calls,
  ...
  mutate(
    month_num = as.numeric(difftime(month, min(month), units = "days")) / 30
  )

# Vérification parallel trends sur panel mensuel
pre_monthly <- panel_monthly %>% filter(post == 0)

trend_test_monthly <- lm(
  arrest_rate ~ month_num * treated,
  data    = pre_monthly,
  weights = n_calls
)
summary(trend_test_monthly)
```

```{r}
twfe_simple <- feols(
  arrest_rate ~ treated * post | city,
  data    = panel_monthly,
  weights = ~n_calls,
  cluster = ~city
)
...
summary(did_ols)
```
+3.17 percentage points arrest rate in eugene compared to springfield for same period, highly signif ***
